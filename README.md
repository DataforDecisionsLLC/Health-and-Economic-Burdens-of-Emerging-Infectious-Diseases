...
##   •	CODE – This folder contains all Stata and Python code used in the analysis. 
...
## •	GENERAL – This is a compressed folder containing the final Stata outputs that are inputs into the models that conducted the historical analyses along with files related to study country lists and study country naming conventions.
## •	python_output – A compressed folder containing all the intermediate outputs and results that are generated by the Python code and that are not presented in the article or supplemental files. Note that per our license restrictions, we are not able to share raw GBD 2021 data.
## •	stata_output – A compressed folder containing all the intermediate outputs and results that generated by the Stata code and that are not presented in the article or supplemental files. Note that per our license restrictions, we are not able to share raw GBD 2021 data.
## •	LIFE TABLES – A compressed Stata .dta file containing raw life tables. 
## •	LIFE EXPECTANCY – A compressed Stata .dta file containing discounted life expectancies.  
## •	RAW DATA – This folder contains the following compressed subfolders containing raw data used in the analysis:
## -	COUNTRY GROUPS
## -	ECONOMIC DATA
## -	OWID
## -	POPULATION
...
## •	With the caveat that the GBD 2021 data exclusion and redactions noted above will prevent users from running some of our code and replicating all outputs, the order in which the programs files would be run to replicate our analysis is:
